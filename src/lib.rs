...
pub struct Record {
    ...
    pub leader: Vec<char>, // It'd be neat to have this set to 24 characters?  &'a [char; 24],
}

#[derive(Debug, Clone)]
pub struct Field {
    pub tag: String,           // both Control and Data fields
    ...
    pub value: Option<String>, // for Control fields
...
pub fn find_records_with_mismatched_pub_years(raw_records: Vec<Vec<char>>) -> Vec<Record> {
...
pub fn two_years_not_the_same(a: &str, b: &str) -> bool {
...
pub fn clean_year(year: &str) -> String {
    // clean_year.replace(".", "").replace("c", "")
    year.chars().filter(|c| c.is_ascii_digit()).collect()
}

pub fn get_year_estimate(year: &str) -> String {
    year.to_string()
        .replace(".", "")
        .replace("c", "")
        .replace("[", "")
